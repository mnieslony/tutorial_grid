...
cd $CONDOR_DIR_INPUT
#tar -xzf ToolAnalysis_DataDecoder_TrigOverlap_BeamStatus_VME03offset.tar.gz
#tar -xzf ToolAnalysis_DataDecoder_TrigOverlap_BeamStatus_improved4_daylight.tar.gz
#tar -xzf ToolAnalysis_DataDecoderwLAPPD.tar.gz
#tar -xzf DataDecoderwLAPPD_VME03Offset.tar.gz 
tar -xzf DataDecoderwLAPPD_VME03OffsetNegative.tar.gz 
...
then
    rm ${CONDOR_DIR_INPUT}/MyToolAnalysis_EventBuildingLAPPD/${TOOLCHAIN_FILE}/my_files.txt
    echo $FILENAME > ${CONDOR_DIR_INPUT}/MyToolAnalysis_EventBuildingLAPPD/${TOOLCHAIN_FILE}/my_files.txt
fi
...
# setup software
export TOOLANALYSIS_PATH=${CONDOR_DIR_INPUT}/MyToolAnalysis_EventBuildingLAPPD
...
# Copy trigoverlap files to ToolAnalysis
ifdh cp -r $CONDOR_DIR_INPUT/$TRIGOVERLAP $TOOLANALYSIS_PATH
cd $TOOLANALYSIS_PATH
tar -xzf $TRIGOVERLAP
echo "Contents of TOOLANALYSIS_PATH:"
ls

ifdh cp -r $CONDOR_DIR_INPUT/$BEAMSTATUS $TOOLANALYSIS_PATH
